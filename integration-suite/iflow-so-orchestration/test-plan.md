...
| ORCH-003 | SAP business validation error | SAP_BUSINESS_ERROR; no blind retry; failed context prepared; DLQ envelope available when routed | Sales area 1000 10 00 does not exist | PASS |
...
| ORCH-012 | DLQ replay validation | DLQ payload contains original SAP JSON, correlationId, idempotencyKey as empty string when missing, consumerId, error context, and replay instruction | TBD | TBD |
